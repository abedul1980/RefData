...
# Clear reference DB
psql ${REFDB_URL} -c "drop database if exists ${REFDB_DBNAME};"

# Clear roles and schemas
psql ${ROOT_URL} -c "drop table if exists flyway_schema_history; drop role if exists ${GOVERNANCE_AUTHENTICATORUSER}; drop role if exists ${REFERENCE_AUTHENTICATORUSER}; drop role if exists ${GOVERNANCE_ANONUSER}; drop role if exists ${GOVERNANCE_SERVICEUSER}; drop role if exists ${GOVERNANCE_READONLYUSER}; drop role if exists ${GOVERNANCE_OWNERNAME}; drop role if exists ${REFERENCE_OWNERNAME}; drop role if exists ${REFERENCE_ANONUSER}; drop role if exists ${REFERENCE_SERVICEUSER}; drop role if exists ${REFERENCE_READONLYUSER};"
